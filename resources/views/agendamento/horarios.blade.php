    ...
    <form id="agendamentoForm" method="POST" action="{{ route("agendamento.confirmar", [$empresa, $servico]) }}">
        ...
        <div id="dadosCliente" style="display: none; margin-top: 2rem; padding-top: 1.5rem; border-top: 2px solid #e2e8f0;">
            ...
            @if($telefoneLogado)
                ...
                <div id="horarioSelecionado" style="background: #e6fffa; border: 1px solid #81e6d9; border-radius: 8px; padding: 1rem; margin: 1rem 0;">
                ...
                <button type="button" class="btn btn-secondary" onclick="cancelarSelecao()" style="margin-left: .5rem;">
            ...
            @else
                ...
                <button type="submit" class="btn btn-success" id="btnConfirmarAgendamento" style="display: none; margin-left: .5rem;">
                    Confirmar Agendamento
                </button>
...
function selecionarHorario(elemento) {
    ...
    if (telefoneLogado) {
        ...
        if (tel) { tel.value = telefoneLogado; tel.setAttribute("readonly", true); }
        ...
        fetch("/api/check-telefone", {
            ...
            body: JSON.stringify({ telefone: telefoneLogado })
        })
        .then(r => r.json())
        .then(data => {
            if (data && data.success && data.exists && data.nome) {
                const nome = document.getElementById("nome_cliente");
                if (nome) { nome.value = data.nome; nome.setAttribute("readonly", true); }
            }
        })
        .catch(() => {});
    } else {
        ...
        const nome = document.getElementById("nome_cliente");
        if (nome) { nome.removeAttribute("required"); nome.removeAttribute("readonly"); nome.value = ""; }
        const nomeGroup = document.getElementById("nomeClienteGroup");
...
function cancelarSelecao() {
    ...
    const nome = document.getElementById("nome_cliente");
    if (nome) { nome.value = ""; nome.removeAttribute("required"); nome.removeAttribute("readonly"); }
    const tel = document.getElementById("telefone_cliente");
    if (tel && !telefoneLogado) { tel.value = ""; }
    const btnVerificar = document.getElementById("btnVerificarTelefone");
    if (btnVerificar) btnVerificar.style.display = "inline-block";
    const btnConfirm = document.getElementById("btnConfirmarAgendamento");
    if (btnConfirm) btnConfirm.style.display = "none";

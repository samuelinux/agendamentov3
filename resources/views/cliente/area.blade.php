    ...
    @else
        <div style="margin-bottom: 0.2rem;">
            <div style="display: flex; justify-content: space-between; align-items: center;">
                ...
                <div>
                    <form method="POST" action="{{ route('cliente.logout', $empresa) }}" style="margin: 0;">
                        @csrf
                        <button type="submit" class="btn btn-secondary" style="font-size: 0.9rem; padding: 0.2rem 0.2rem;">
                            Sair
                        </button>
                    </form>
                </div>
                ...
                <div style="display: flex; justify-content: between; align-items: flex-start; margin-bottom: 0.2rem;">
                    ...
                    <div style="text-align: right;">
                        @if ($agendamento->status === 'confirmado')
                            @if ($podeSerCancelado)
                                <span
                                    style="background: #48bb78; color: white; padding: 0.25rem 0.75rem; border-radius: 20px; font-size: 0.8rem; font-weight: 500;">
                                    Confirmado
                                </span>
                            @elseif($jaPassou)
                                <span
                                    style="background: #4299e1; color: white; padding: 0.25rem 0.75rem; border-radius: 20px; font-size: 0.8rem; font-weight: 500;">
                                    Realizado
                                </span>
                            @endif
                        @elseif($agendamento->status === 'cancelado')
                            <span
                                style="background: #f56565; color: white; padding: 0.25rem 0.75rem; border-radius: 20px; font-size: 0.8rem; font-weight: 500;">
                                Cancelado
                            </span>
                        @endif
                    </div>
